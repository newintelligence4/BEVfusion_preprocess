...
#include <vector>
#include <time.h>
#include "right_lidar/right_lidar_node.hpp"
...
int speed = 0;
double param_car_size;
RIGHT_PREPROCESSING::RIGHT_PREPROCESSING(const rclcpp::NodeOptions& options) : Node("right_lidar",options) 
{
      
  declare_parameter<std::string>("topic_pointcloud_in","/left_lidar");
  declare_parameter<std::string>("topic_pointcloud_out", "/points_raw2");
  declare_parameter<double>("car_size_", rclcpp::PARAMETER_DOUBLE);
  ...
  param_car_size = get_parameter("car_size_").as_double();
  
  publisher_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(param_topic_pointcloud_out,2);
  subscription_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
  param_topic_pointcloud_in, 10, std::bind(&RIGHT_PREPROCESSING::topic_callback, this, _1));
  ...
  param_topic_velocity_in, 10, std::bind(&RIGHT_PREPROCESSING::velocity_callback, this, _1));

}
void RIGHT_PREPROCESSING::velocity_callback(const velocity_msg::msg::Num::SharedPtr vel)
{
  speed = vel->num;
}
void RIGHT_PREPROCESSING::topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
...
pcl::PCLPointCloud2 adaptive_roi(const pcl::PCLPointCloud2 cloud, int velocity)
{
  ...
  double car_size = param_car_size;
  
  float road_x = (-0.5)*car_size;
  float road_y = (-1)*car_size;
  ...
  pass.setFilterLimits(road_x, +10.5);
  ...
  for(unsigned int j=0; j<boundery->points.size(); j++)
  {
    ...
    //   std::cerr << "Limit angle: "<< angle_h << std::endl; 
    // }
    if(angle_h > 150)
    {
        boundery->points[j].x = 0;
...
double get_cos(double x, double y)
{
  ...
  //std::cout<<"Hey!"<<theta<<std::endl;
  if (acos(x/r)==0){ std::cout<<"Hey!"<<theta<<"\n"<<std::endl;}
  return theta;
}     
double get_tan(double x, double y, double z)
{
