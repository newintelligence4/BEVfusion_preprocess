...
BACK_PREPROCESSING::BACK_PREPROCESSING(const rclcpp::NodeOptions& options) : Node("back_lidar",options) 
{
      
  declare_parameter<std::string>("topic_pointcloud_in","/left_lidar");
  declare_parameter<std::string>("topic_pointcloud_out", "/points_raw0");
  ...
  param_topic_pointcloud_in, 10, std::bind(&BACK_PREPROCESSING::topic_callback, this, _1));
  ...
  param_topic_velocity_in, 10, std::bind(&BACK_PREPROCESSING::velocity_callback, this, _1));

}

void BACK_PREPROCESSING::velocity_callback(const velocity_msg::msg::Num::SharedPtr vel)
{
  speed = vel->num;
}

void BACK_PREPROCESSING::topic_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  output.header.frame_id = "back_lidar_link";
...
pcl::PCLPointCloud2 adaptive_roi(const pcl::PCLPointCloud2 cloud, int velocity)
{
  ...
  pass.setFilterLimits(-13.0, 13.0);
  pass.filter(*boundery);

  for(unsigned int j=0; j<boundery->points.size(); j++)
  {
    double angle_h = get_cos((boundery->points[j].y - road_y) , (boundery->points[j].x));
